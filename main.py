...
ALERT_TYPES = [
    ...
    ("daily_briefing", "Daily Briefing & Bias"),
...
async def crt_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for pair in pairs:
        for tf_label, tf_interval in CRT_TIMEFRAMES.items():
            ...
            if not candles:
                logger.warning(f"/crt: no data for {pair} {tf_label}")
                continue
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "All commands:\n\n"
        "/log PAIR — log a trade (guided: session, setup, RR, confidence, result)\n"
        "/history — your last 10 trades\n"
        "/stats — all-time performance\n"
        "/weekly — this week's performance\n"
        "/monthly — this month's performance\n"
        "/watchlist — show your watched pairs\n"
        "/watchlist add PAIR [PAIR2 ...] — add pairs to scan\n"
        "/watchlist remove PAIR — remove a pair\n"
        "/crt — check watchlist for confirmed CRT setups now\n"
        "/crt PAIR — check just one pair\n"
        "/mute — toggle automatic messages on/off\n"
        "/cancel — cancel a /log in progress\n"
        "/ping — check the bot is alive\n"
        "/help — this list\n\n"
        "Automatic (mutable via /mute):\n"
        "• Weekly report — Sundays\n"
        "• Monthly report — 1st of each month\n"
        "• Watchlist scan — breakout/sweep/retest/CISD, every 4h\n"
        "• CRT scan — 4H/1H every 4h, 1W/1D once daily\n"
        "• Daily briefing & bias — once daily\n"
        "• High-impact news alerts — checked periodically"
    )
...
async def send_daily_briefing(context: ContextTypes.DEFAULT_TYPE):
    """Runs once daily. Builds a market bias from news + calendar and sends it to opted-in users."""
    news_items = news_client.fetch_general_news(limit=8)
    headlines = [n.get("headline", "") for n in news_items if n.get("headline")]
    calendar_events = news_client.fetch_economic_calendar(days_ahead=1)

    bias_text = reasoning_client.generate_bias(headlines, calendar_events)
    if not bias_text:
        # Fall back to raw headlines if the reasoning API isn't configured/failed,
        # so users still get something useful rather than silence.
        bias_text = "Bias reasoning unavailable right now. Recent headlines:\n" + \
                     ("\n".join(f"- {h}" for h in headlines) if headlines else "No headlines available.")

    message = f"📰 Daily Briefing & Bias\n\n{bias_text}"

    for user_id in db.get_all_user_ids():
        if not db.is_alert_enabled(user_id, "daily_briefing"):
            continue
        try:
            await context.bot.send_message(chat_id=user_id, text=message)
        except Exception as e:
            logger.warning(f"Failed to send daily briefing to {user_id}: {e}")


# ---------- high-impact news alerts ----------

async def check_high_impact_news(context: ContextTypes.DEFAULT_TYPE):
    """Runs periodically. Alerts opted-in users about high-impact events starting soon."""
    events = news_client.fetch_high_impact_events_soon(hours_ahead=1)
    if not events:
        return
    ...
    for event in events:
        ...
        dedupe_key = f"news_{event_name}_{event_time}"

        for user_id in db.get_all_user_ids():
            if not db.is_alert_enabled(user_id, "news_alert"):
                continue
            if db.already_alerted(user_id, "NEWS", dedupe_key, event_time):
                continue
            try:
                await context.bot.send_message(
                    chat_id=user_id,
                    text=f"⚠️ High-impact news soon: {event_name} ({country}) at {event_time}"
                )
                db.mark_alerted(user_id, "NEWS", dedupe_key, event_time)
            except Exception as e:
                logger.warning(f"Failed to send news alert to {user_id}: {e}")
...
def main():
    ...
    # Daily briefing & bias — sent shortly before market open (10:30pm WAT / 21:30 UTC).
    app.job_queue.run_daily(send_daily_briefing, time=time(hour=21, minute=30))
    # High-impact news — checked every 15 minutes so alerts land close to "1 hour before" the event.
    app.job_queue.run_repeating(check_high_impact_news, interval=900, first=30)
